        ...
        var compileWalker = function compileWalker(tree) {
            return tree.map(function (branch, branchIndex) {
                ...
                switch (branch.type) {
                    ...
                    case 'phrase':
                        if (branch.field) {
                            buffer += branch.field == 'title' ? 'TITLE(' + _tools2.default.quotePhrase(branch, 'scopus', settings.highlighting) + ')' : branch.field == 'abstract' ? 'ABS(' + _tools2.default.quotePhrase(branch, 'scopus', settings.highlighting) + ')' : branch.field == 'title+abstract' ? 'TITLE-ABS(' + _tools2.default.quotePhrase(branch, 'scopus', settings.highlighting) + ')' : branch.field == 'title+abstract+tw' ? 'TITLE-ABS(' + _tools2.default.quotePhrase(branch, 'scopus', settings.highlighting) + ')' : branch.field == 'title+abstract+other' ? 'TITLE-ABS-KEY(' + _tools2.default.quotePhrase(branch, 'scopus', settings.highlighting) + ')' : branch.field == 'floatingSubheading' ? 'INDEXTERMS(' + _tools2.default.quotePhrase(branch, 'scopus', settings.highlighting) + ')' : branch.field == 'publicationType' ? 'DOCTYPE(' + _tools2.default.quotePhrase(branch, 'scopus', settings.highlighting) + ')' : branch.field == 'substance' ? 'CHEM(' + _tools2.default.quotePhrase(branch, 'scopus', settings.highlighting) + ')' : branch.field == 'language' ? 'LANGUAGE(' + _tools2.default.quotePhrase(branch, 'scopus', settings.highlighting) + ')' : _tools2.default.quotePhrase(branch, 'scopus', settings.highlighting);
                        } else {
                            if (settings.highlighting) {
                                buffer += _tools2.default.createPopover(_tools2.default.quotePhrase(branch, 'scopus', settings.highlighting), branch.offset + branch.content.length);
                            } else {
                                buffer += _tools2.default.quotePhrase(branch, 'scopus', settings.highlighting);
                            }
                        }
                        break;
                    case 'joinAnd':
                    ...
                    case 'mesh':
                        if (settings.highlighting) {
                            buffer += _tools2.default.createTooltip('<font color="blue">' + 'INDEXTERMS("' + branch.content + '")</font>', "Polyglot does not translate subject terms (e.g Emtree to MeSH), this needs to be done manually");
                        } else {
                    ...
                    case 'raw':
                        ...
                        break;
                    case 'template':
                        buffer += _tools2.default.resolveTemplate(branch.content, 'scopus');
                        break;
                    case 'comment':
                        // Do nothing
                        break;
