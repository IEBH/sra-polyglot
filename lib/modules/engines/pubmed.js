        ...
        var compileWalker = function compileWalker(tree) {
            return tree.map(function (branch, branchIndex) {
                ...
                switch (branch.type) {
                    ...
                    case 'meshMajor':
                        if (settings.highlighting) {
                            buffer += _tools2.default.createTooltip('<font color="blue">' + _tools2.default.quotePhrase(branch, 'pubmed') + '[Majr' + (branch.recurse ? '' : ':NoExp') + ']</font>', "Polyglot does not translate subject terms (e.g Emtree to MeSH), this needs to be done manually");
                        } else {
                            buffer += _tools2.default.quotePhrase(branch, 'pubmed') + '[Majr' + (branch.recurse ? '' : ':NoExp') + ']';
                        }
                        break;
                    case 'raw':
