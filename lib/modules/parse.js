...
*/
var parse = exports.parse = function parse(query, options) {
    ...
    _global2.default.variables.no_field_tag = []; // Empty array of offsets each time the query is parsed
    ...
    if (settings.transposeLines || settings.groupLines || settings.removeNumbering) {
        ...
        if (settings.groupLines && (settings.groupLinesAlways || lines.length > 1)) {
            ...
            lines = lines.map(function (line) {
                return _lodash2.default.trim(line) && !/^\s*(and|or|not)\s*$/i.test(line) && !/^\s*#/.test(line) ? '(' + line + ')' : line;
            });
        }
        // }}}
    ...
    */
    function trimLastLeaf() {
        if (leaf && _lodash2.default.includes(['phrase', 'raw'], leaf.type) && / $/.test(leaf.content)) {
    ...
    */
    ...
    while (q.length) {
        ...
        if (/^\(/.test(q)) {
            ...
            branch.nodes.push({
                type: 'ref',
                ref: _lodash2.default.range(match[1], (match[2] + 1) / 10),
            ...
            branch.nodes.push({
                type: 'ref',
                ref: _lodash2.default.range(match[2], (match[3] + 1) / 10),
            ...
            cropString = false;
        } else if (match = /^(AND|OR|NOT) +([0-9]+)\s+/i.exec(q)) {
            // AND 2...
            trimLastLeaf();
            ...
            trimLastLeaf();
            branch.nodes.push({ type: 'joinNear', proximity: _lodash2.default.toNumber(match[2]) });
            leaf = undefined;
            offset += match[0].length;
            q = q.substr(match[0].length);
            cropString = false;
        ...
        || (match = /^:(tw|ti,ab|ab,ti|ti|ab|mp|nm|pt|fs|sh|xm|af|lg)/i.exec(q)) // term:INITIALS
                ...
                var useLeaf = {};
                if (_lodash2.default.isObject(leaf) && leaf.type == 'phrase') {
                    useLeaf = leaf;
                } else if (_lodash2.default.isArray(leaf) && lastGroup) {
                    useLeaf = lastGroup;
                }
                // }}}
                ...
                q = q.substr(match[0].length);
                cropString = false;
            } else if (match = /^\[(tiab|title\/abstract|ti|title|tw|ab|nm|sh|pt|all|all fields|la|language)\]/i.exec(q)) {
            ...
            if ((_lodash2.default.isUndefined(leaf) || _lodash2.default.isArray(leaf)) && nextChar != ' ') {
            ...
            } else if (_lodash2.default.isObject(leaf) && leaf.type == 'phrase') {
    ...
    if (settings.transposeLines) {
        _tools2.default.visit(tree.nodes, ['ref'], function (node, path) {
            var reference;
            var line;
            ...
            for (reference in node.ref) {
                ...
                node.nodes.push([{
                    type: "phrase",
                    content: _tools2.default.createTooltip("Line " + node.ref[reference] + " not found", "Polyglot could not find specified line in the search query", "red-underline")
                }]);
            }
        });
    }
