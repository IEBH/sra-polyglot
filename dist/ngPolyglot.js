		...
		parse: function parse(query, options) {
			...
			if (settings.groupLines) {
				...
				if (settings.groupLinesAlways || lines.length > 1) {
					...
					.map(function (line) {
						return _.trim(line) && !/^\s*(and|or|not)\s*$/i.test(line) ? '(' + line + ')' : line;
					}).join('\n');
				}
			}
