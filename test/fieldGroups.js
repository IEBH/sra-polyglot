...
describe('Translate field groups (Ovid -> CINAHL)', ()=> {
	...
	it('translate OV `(term1 OR term2).tw.` -> CI `(TI term1 OR AB term1 OR TI term2 OR AB term2)`', ()=> {
		expect(polyglot.translate('(term1 OR term2).tw.', 'cinahl')).to.equal('(TI term1 OR AB term1 OR TI term2 OR AB term2)');
	});

	it('translate OV `((term1 OR term2) AND term3).tw.` -> CI `(TI term1 OR AB term1 OR TI term2 OR AB term2)`', ()=> {
